...
class RNNTrainer:
    ...
    def fit(self, X, y_true, batch_size, lr, n_epochs, print_many=False):
        """
        X = np.array([
            [[1, 2], [4, 6], [7, 9], [5, 1]],
            [[2, 1], [7, 9], [8, 9], [1, 2]],
        ...
        self.train(X, y_true, batch_size, lr, n_epochs, print_many)
    ...
    def train(self, X, y_true, batch_size, learning_rate, num_epochs, print_many):
        self.batch_size_global = batch_size
        ...
        for epoch in range(num_epochs):
            ...
            if (print_many and epoch % 100 == 0) or (not print_many and epoch in progresses):
                print(f"after epoch: {epoch}, epoch_losses: {round(np.mean(epoch_losses), 3)}")
